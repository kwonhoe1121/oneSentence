...
public class SearchController {
	...
	public String getSearchMoreSentenceList(HttpServletRequest request, Model model, @RequestParam String query)
			throws Exception {
	...
	public String getSearchMoreUser(HttpServletRequest request, Model model, @RequestParam String query)
			throws Exception {
		List<UserVo> userInfo = serviceUser.selectUserByUserName('%' + request.getParameter("query") + '%');
		model.addAttribute("userInfo", userInfo);
	...
	public String getContentsBook(HttpSession session, HttpServletRequest request, Model model,
		...
		if (isbn != null) {
			...
			if (user != null) {
				...
				if (sentence != null) {
					...
					oneSentenceList.add(sentence);
					oneSentenceList.addAll(oneService
							.selectOnesentenceListByoneSentenceIdxAndIsbnOther(user.getUserIdx(), user2, isbn));
				} else {
					oneSentenceList.addAll(oneService
							.selectOnesentenceListByoneSentenceIdxAndIsbnOtherNoFollwing(user.getUserIdx(), isbn));
				}
			} else {
